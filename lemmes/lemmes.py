          ...
          if token in lemmas:
            ...
            if not lemma:
              for x in temp_lemma:
                if x in lemmas:
                  lemma = x
                  self.LOG.info(token + ' in_lemmas: ' + repr(x))

            if not lemma:
              aprox = difflib.get_close_matches(stemmed, lemmas, 2)
              if len(aprox):
                lemma = aprox
              self.LOG.info(token + ' difflib: ' + repr(lemma))
            self.LOG.info(token + ' check: ' + repr(lemma))
      else:
        lemma = token if stemm_unk else stemmed
